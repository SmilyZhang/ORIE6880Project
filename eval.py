...
#
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            print("batch number {}/{}".format(i+1, len(data_loader)))
            input_img = data[0]  # [batch_size, 3, height, width]
            labels = data[1].cuda()  # [batch_size]
...
#
    ...
    for v_id, val in enumerate(vals):
        ...
        data_loader = create_dataloader_test(opt)
        accuracy, cm, report = validate(model, data_loader)
        results.append([val, accuracy, cm, report])
        print(f"Accuracy: {accuracy}; Confusion Matrix: \n{cm}\nReport: \n{report}")

    csv_name = f'{results_dir}/{model_name}.csv'
    with open(csv_name, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Dataset', 'Accuracy', 'Confusion Matrix', 'Classification Report'])
